...
import { BUSINESS_TYPE_OPTIONS, BUSINESS_TYPE_MAP } from '@/constants/common';

import { FUND_PAYMENT_TYPE, FUND_PAYMENT_MAP } from '@/pages/ClientManagementInfo/constants';
import { PAYMENT_STATUS_OPTIONS, PAYMENT_STATUS_ZHCN_MAP } from '../constants';
...
export const CASHFLOW_TYPE_MAP = {
  PREMIUM_ADVANCE: '前端期权费',
  PREMIUM_LAST: '后端期权费',
  PAYMENT_OPTION: '期权结算支付金额',
  PAYMENT_ADVANCE: '前端收益',
  PAYMENT_LAST: '后端收益',
  PRE_PAYMENT_INTEREST: '预付金返息',
  PAYMENT: '支付',
  INTEREST_INCOME: '利率收益',
  EQUITY_INCOME: '权益收益',
};
        ...
        {form.getFieldDecorator({})(
          <Select
            ...
            options={async (value: string = '') => {
              const {
                data: { result, error },
              } = await fetch['POST/capital-service/api/rpc/method=capTradeSerialIdSearch']({
                searchKey: value || '',
              });
              if (error) return [];
              return _.uniqBy(
        ...
        {form.getFieldDecorator({})(
          <Select
            ...
            options={async (value: string = '') => {
              const {
                data: { result, error },
              } = await fetch['POST/capital-service/api/rpc/method=capTradeTradeIdSearch']({
                searchKey: value || '',
              });
              if (error) return [];
              return _.uniqBy(
...
export const TABLE_COLUMNS = [
  ...
  },
  {
    title: '实付状态',
    dataIndex: 'paymentStatus',
    render: value => PAYMENT_STATUS_ZHCN_MAP[value],
  },
  {
    title: '备注',
    dataIndex: 'comment',
