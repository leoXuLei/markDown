...
**【查询最近 10 条非合并提交的日志，并以特定格式输出每条日志的信息。】**

```bash
git log -10 --no-merges --dense --format="%n>>%h | %ai | %s"
```

- `%n`：表示换行符。
- `>>`：这是一个自定义的分隔符，用于标识每条日志的开始。
- `%h`：表示提交的简短哈希值。
- `%ai`：表示提交的作者日期（ISO 8601 格式）。
- `%s`：表示提交的简短说明。
