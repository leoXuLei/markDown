...
## RegExp.prototype.exec()：匹配结果保护组匹配的内容
...
需求：获取数字字符串有效数字的位数（即去除数字字符串中的小数点和末尾无效的零）
```js
('4.03400').replace(/\.(\d*)0+$/, "$1"); 
// '40340'

// \d*后面加了?，表示非贪婪模式，即尽可能少的匹配数字（最少就是没有数字）
('4.03400').replace(/\.(\d*?)0+$/, "$1");
// '4034'
...
('401.00').replace(/\.(\d*?)0+$/, "$1");
// '401'

('401.0340').replace(/\.(\d*?)0+$/, "$1");
// '401034'



('401.3402').replace(/\.(\d*?)0+$/, "$1");
// '401.3402'
// 因为末尾是`0+`，即0出现1次或多次，导致没有匹配上，所以没有replace成功


('401.3420').replace(/\.(\d*?)0+$/, "$1");
// '401342'
...
**【定义：】**
正则表达式的括号表示分组匹配，括号中的模式可以用来匹配分组的内容。（括号中的内容作为一个整体，并且记住这个整体）
...
### 以下是零宽断言，自身不会匹配返回内容，只是为了定位辅助匹配的
### **3.先行断言:`x(?=y)`（正向预查）**
用法是`exp1(?=exp2)`，**查找出现在exp2前面的exp1，exp2是不会出现在结果里的**。
...
`?!`和`?=`是一对，`?!`是`?=`的否定语义，用法是`exp1(?!exp2)`，其意义是**不是出现在exp2前面的exp1**。
...
```js
var m = "abac".match(/(?<=b)c/g);
m; // null

var m = "abcac".match(/(?<=b)c/g);
m // ['c']

var m = "abcacabc".match(/(?<=b)c/g);
m // ['c', 'c']
```
...
var m = "abcbcabc".match(/(?<!b)c/g);
m; // null
```

**【参考链接】：**

- [@@正则表达式的高级用法——正|反向预查](https://zhuanlan.zhihu.com/p/404880938)
