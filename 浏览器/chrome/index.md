...
先将目标父元素声明为全局变量`temp1`，再将要拷贝的目标子元素声明为全局变量`temp2`，然后执行如下代码：`temp1.appendChild(temp2.cloneNode(true))`，即表示赋值一次，如想复制很多次，写一个循环即可，`[...Array(7)].forEach(() => { temp2.appendChild(temp1.cloneNode(true)) })`
